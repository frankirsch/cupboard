...
public class PantryProvider extends ContentProvider {
    ...
    static {
        ...
        // integer code {@link #FOOD}. This URI is used to provide access to MULTIPLE rows
        // of the pets table.
        ...
        sUriMatcher.addURI(PantryContract.CONTENT_AUTHORITY, PantryContract.PATH_FOOD + "/#", FOOD_ID);
    }
    ...
    public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs,
        ...
        switch (match) {
            case FOOD:
                // For the FOOD code, query the pets table directly with the given
    ...
    private Uri insertFood(Uri uri, ContentValues values) {
        ...
        Integer unit = values.getAsInteger(FoodEntry.COLUMN_FOOD_AMOUNT_UNIT);
        if (unit == null || !FoodEntry.isValidUnit(unit)) {
            throw new IllegalArgumentException("Food requires valid unit");
        }

        // If the weight is provided, check that it's greater than or equal to 0 kg
        Integer amount = values.getAsInteger(FoodEntry.COLUMN_FOOD_AMOUNT);
        if (amount != null && amount < 0) {
            throw new IllegalArgumentException("Food requires valid amount");
        }
        ...
        long id = database.insert(PantryContract.FoodEntry.TABLE_NAME, null, values);
    ...
    private int updateFood(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        ...
        // If the {@link FoodEntry#COLUMN_FOOD_AMOUNT} key is present,
        // check that the gender value is valid.
        if (values.containsKey(FoodEntry.COLUMN_FOOD_AMOUNT_UNIT)) {
            Integer unit = values.getAsInteger(FoodEntry.COLUMN_FOOD_AMOUNT_UNIT);
            if (unit == null || !FoodEntry.isValidUnit(unit)) {
                throw new IllegalArgumentException("Food requires a valid unit");
            }
        }

        // If the {@link FoodEntry#COLUMN_FOOD_AMOUNT_UNIT} key is present,
        // check that the weight value is valid.
        if (values.containsKey(FoodEntry.COLUMN_FOOD_AMOUNT)) {
